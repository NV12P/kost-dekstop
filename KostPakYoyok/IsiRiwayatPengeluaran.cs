...
namespace KostPakYoyok
{
    public partial class IsiRiwayatPengeluaran : UserControl
    {
        ...
        public IsiRiwayatPengeluaran()
        {
            ...
            this.Load += IsiRiwayatPengeluaran_Load;
        ...
        // =========================
        // RESET PILIHAN (Tanpa trigger event)
        // =========================
        ...
        private void SelectCard(Guna2Panel card)
        {
            ...
            // Reset state internal tanpa sebar gosip (trigger event) dulu
            InternalDeselect();
            ...
            // Pastikan Tag ada isinya
            if (card.Tag != null)
                SelectedId = Convert.ToInt32(card.Tag);

            var lblKet = card.Controls.OfType<Label>()
                .FirstOrDefault(x => x.Name == "lblKet");

            var lblNom = card.Controls.OfType<Label>()
                .FirstOrDefault(x => x.Name == "lblNom");

            if (lblKet != null)
                SelectedKeterangan = lblKet.Text;
            ...
            // Baru deh sebar gosip kalau data udah siap
            OnSelectionChanged();
        }
            ...
            try
            {
                ...
                using (var client = new HttpClient())
                {
                    ...
                    var arr = JArray.Parse(json)
                        .Where(x => x["tipe"]?.ToString() == "pengeluaran") // Filter cuma pengeluaran
                        .OrderByDescending(x =>
                            DateTime.Parse(
                    ...
                    {
                        // Handle id_keuangan (contoh: "keluar-1" -> ambil 1)
                        string rawId = item["id_keuangan"]?.ToString() ?? "0";
                        int id = 0;
                        if (rawId.Contains("-"))
                        {
                            int.TryParse(rawId.Split('-').Last(), out id);
                        }
                        else
                        {
        ...
        // BUAT CARD
        // =========================
        private void AddNewCardInternal(
            int id,
            string keterangan,
            string tanggal,
            long nominal
        )
        {
            Guna2Panel card = new Guna2Panel
            {
                ...
                Size = new Size(panelData.Width, panelData.Height),
        ...
        // TAMBAH CARD BARU
        // =========================
        public void AddNewCard(string keterangan, long nominal)
        {
        ...
        public void FilterData(string keyword)
        {
            ...
            foreach (var card in cards)
            {
                ...
                {
                    bool cocok =
                        lblKet.Text.ToLower().Contains(keyword);
        ...
        private void ReflowCards()
        {
            ...
            foreach (var card in cards)
            {
                ...
                if (lblNom != null)
                {
                    lblNom.Location = new Point(
                        card.Width - lblNom.Width - 30,
                        lblNom.Location.Y
                    );
                }
            }
        ...
        // LOAD CONTROL
        // =========================
        private async void IsiRiwayatPengeluaran_Load(
            object sender,
            EventArgs e
        )
        {
